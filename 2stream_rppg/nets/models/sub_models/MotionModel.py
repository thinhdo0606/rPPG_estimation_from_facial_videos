...
if os.getenv("RPPG_DISABLE_TAM", "0") == "1":
    from nets.blocks.blocks_woTAM import TSM_Block, ECA_Block, STM, CSTM, TSM_CSTM, TAM, TSM_Block_Adv, CMM, TSM_CSTM_Serial
else:
...
class MotionModel_TS_CSTM(nn.Module):
    def __init__(self, eca, in_planes, kernel_size, frame_depth, shift_factor, skip, group_on):
        ...
        #-- TMB1 --#
        self.block_1 = self._build_tsm_cstm(in_planes, frame_depth, shift_factor, skip, group_on)
        ...
        #-- TMB2 --#
        self.block_2 = self._build_tsm_cstm(in_planes * 2, frame_depth, shift_factor, skip, group_on)
    ...
    @staticmethod
    def _build_tsm_cstm(in_planes, frame_depth, shift_factor, skip, group_on):
        # `blocks.py` expects (.., skip, group_on) while `blocks_woTAM.py` expects (.., group_on).
        try:
            return TSM_CSTM(in_planes, frame_depth, shift_factor, skip, group_on)
        except TypeError:
            return TSM_CSTM(in_planes, frame_depth, shift_factor, group_on)
