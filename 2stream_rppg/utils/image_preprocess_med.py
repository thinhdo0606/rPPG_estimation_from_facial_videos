...
def preprocess_Video_RGB_only_Hao(path, flag, vid_res): # vis_res: image size  path: each video_path in corresponding dataset
    ...
    with tqdm(total=frame_total, position=0, leave=True, desc=path) as pbar:
        ...
        face_detection=mp_facedetector.FaceDetection(min_detection_confidence=0.5)
        while cap.isOpened():
            ret, frame = cap.read()
            if frame is None:
                return False, None
            if flag: # detect face
                # print("Hello workd")
                rst, crop_frame = faceDetection(frame, face_detection)
...
def faceDetection(frame, face_detection):
    global locat

    # resized_frame = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)
    resized_frame = frame
    resized_frame_de = cv2.cvtColor(resized_frame, cv2.COLOR_BGR2RGB)
    # print('resize: ', resized_frame.shape)
    # face_location = face_locations(resized_frame, model='cnn')
    results = face_detection.process(resized_frame_de)
    if results.detections:
        for face in results.detections:
            ...
            top, right, bottom, left = boundBox[1], boundBox[0]+boundBox[2], boundBox[1]+boundBox[3], boundBox[0]
            # print(face_location[0])
            dst = resized_frame[max(0, top - 24):min(resized_frame.shape[0], bottom),  # Y axis: top-10(bias) of iamge or 0 to bottom+10(bias) or original_img_y
                max(0, left - 12):min(resized_frame.shape[1], right + 12)]   # X-axis top_left(bias) of image or 0 to top_right+10(boas) or origina_img_x            
            #-- original code --#
