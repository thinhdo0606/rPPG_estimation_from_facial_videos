...
target_root_path: str = "Baseline_Size72_BPF_0.67_2/"
refe_root_path: str = "Baseline/"
file_target = h5py.File(target_root_path + dataset_name + ".hdf5", "r")
file_refe = h5py.File(refe_root_path + dataset_name + ".hdf5", "r")
data_list = file_target.keys()
data_list = list(data_list)

def preprocessing(init: bool = True,
                  dataset_root_path: str = "DATASETS/",
                #   save_root_path: str = "Baseline_Size72_BPF_0.67_2_short/"):
...
def preprocess_dataset(path, flag, data_path, dataset_name, return_dict):
    ...
    refe_len = len(file_refe[data_path]["video"])
    video = file_target[data_path]["video"][:refe_len]
    if dataset_name == 'MANHOB_HCI':
        label = file_target[data_path]["label"][:refe_len]
        # label = file_refe[data_path]["label"]
