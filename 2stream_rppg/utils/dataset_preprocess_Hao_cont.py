...
from image_preprocess_nresize import preprocess_Video_RGB_only_Hao
# from image_preprocess_med import preprocess_Video_RGB_only_Hao
from text_preprocess import *
import cv2


def preprocessing(init: bool = True,
                  save_root_path: str = "Baseline_Size72/",
    ...
    if dataset_name == "PURE":
        ...
        file_re = h5py.File(save_root_path + dataset_name + ".hdf5", "a")
        existing_keys = set(file_re.keys())
        # print(file_re.keys())
        dataset_root_path = data_root_path + dataset_name
        data_list_pre = [data for data in natsort.natsorted(os.listdir(dataset_root_path)) if data.isdigit()]
        data_list = [data for data in data_list_pre if data not in existing_keys]
        # print(len(existing_keys))
        # print(len(data_list_pre))
        # print(len(data_list))
        random.shuffle(data_list)
    else:
...
def preprocess_dataset(path, flag, dataset_name, return_dict, img_size):
    if dataset_name == 'UBFC':
        ...
        # Debug
        # cap = cv2.VideoCapture(path + "/vid.avi")
        # frame_total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))  # cap.get() -> get the total frame of video
        # rst, video  = True, np.empty((frame_total, 1, 1, 3), dtype=np.uint8) # empty array for all the frame
        rst, video = preprocess_Video_RGB_only_Hao(path + "/vid.avi", flag, vid_res=img_size)
        if not rst:
            return
        else:
