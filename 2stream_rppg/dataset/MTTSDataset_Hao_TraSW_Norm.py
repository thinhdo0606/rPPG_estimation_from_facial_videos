...
class MTTSDataset_TraSW_Norm(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # motion_frames_dip = torch.empty_like(motion_frames)
        # motion_frames_norm = torch.empty_like(motion_frames)
        epsilon = 1e-5
        for i in range(self.window_length):

            # motion_frames[i] = (x[i + 1] - x[i])
            motion_frames[i] = torch.abs(x[i+1] - x[i]) / torch.abs(x[i+1] + x[i]) + epsilon
            # motion_frames_mean = torch.mean(motion_frames[i])
            # motion_frames_std = torch.std(motion_frames[i])
            # motion_frames[i] = torch.stack((motion_frames_mean, motion_frames_std), dim=1)
            # std = torch.std(motion_frames[i], dim=0)
            # mean = torch.mean(motion_frames[i], dim=0)
            # motion_frames_ipmax = mean + 3*std
            # motion_frames_norm[i] = motion_frames[i]-mean / std
            # motion_frames_dip[i] = torch.max(motion_frames[i], motion_frames_ipmax)

        average_frame = x[:-1]      # Dao
        # combined_frames = torch.cat((motion_frames, motion_frames_norm), dim=0)  # 在 channel 维度拼接
